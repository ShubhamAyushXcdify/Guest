...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ message: `Error fetching symptoms: ${error.message}` }, { status: 500 });
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ message: `Error creating symptom: ${error.message}` }, { status: 500 });
    }
} 