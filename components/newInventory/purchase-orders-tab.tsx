            ...
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                {isLoading ? (
                  ...
                  <tr><td colSpan={6} className="text-center py-6">No purchase orders found.</td></tr>
                  ...
                  <tr key={order.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{order.orderDate ? new Date(order.orderDate).toLocaleDateString() : "-"}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className="px-2 py-1 text-xs font-medium bg-gray-100 text-gray-800 rounded-full">{order.status}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{order.totalAmount ? `$${order.totalAmount.toLocaleString(undefined, {minimumFractionDigits:2})}` : "-"}</td>
