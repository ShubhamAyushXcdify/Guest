...
interface EuthanasiaCertificateProps {
...
export default function EuthanasiaCertificate({ appointmentId, patientId, onClose, readOnly = false }: EuthanasiaCertificateProps) {
  const [isGenerating, setIsGenerating] = useState(false)
  const [isSaved, setIsSaved] = useState(false)
  const [petDescription, setPetDescription] = useState("The pet was treated with dignity and compassion throughout the process")
    ...
    try {
      ...
      if (certificateData) {
        ...
        toast({
          ...
          description: "Euthanasia Certificate updated successfully.",
        });
      } else {
        ...
        toast({
          ...
          description: "Euthanasia Certificate saved successfully.",
        });
      }
    } catch (error: any) {
      toast({
  ...
  const handleDownloadCertificate = async () => {
    try {
      ...
      const EuthanasiaPDF = () => (
        <Document>
          <Page size="A4" style={certificateStyles.page}>
            ...
            {/* Title */}
            <Text style={certificateStyles.title}>EUTHANASIA CERTIFICATE</Text>

            {/* Main Content */}
            <Text style={certificateStyles.content}>
              This is to certify that I have personally performed humane euthanasia on the below mentioned pet {getPatientDisplayName()} in accordance with veterinary standards and protocols. The procedure was conducted with the utmost care and compassion, ensuring the pet's comfort and dignity throughout the process.
            </Text>
      ...
      await downloadPDF(
        <EuthanasiaPDF />,
        `euthanasia-certificate-${getPatientDisplayName()}-${new Date().toISOString().split('T')[0]}.pdf`,
        () => {
          toast({
            title: "Success",
            description: "Euthanasia Certificate downloaded successfully",
          });
        },
        (error) => {
          toast({
  ...
  const certificationText = "This is to certify that I have personally performed humane euthanasia on the below mentioned pet. The procedure was conducted in accordance with veterinary standards and regulations. The pet was treated with dignity and compassion throughout the process."
