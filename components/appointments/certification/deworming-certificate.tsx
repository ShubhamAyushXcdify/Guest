...
interface DewormingCertificateProps {
...
export default function DewormingCertificate({ appointmentId, patientId, onClose, readOnly = false }: DewormingCertificateProps) {
    ...
    try {
      ...
      if (certificateData) {
        ...
        toast({
          ...
          description: "Deworming Certificate updated successfully.",
        });
      } else {
        ...
        toast({
          ...
          description: "Deworming Certificate saved successfully.",
        });
      }
    } catch (error: any) {
      toast({
  ...
  const handleDownloadCertificate = async () => {
    try {
      ...
      const DewormingPDF = () => (
        <Document>
          <Page size="A4" style={certificateStyles.page}>
            ...
            {/* Title */}
            <Text style={certificateStyles.title}>DEWORMING CERTIFICATE</Text>

            {/* Main Content */}
            <Text style={certificateStyles.content}>
              This is to certify that I have personally administered deworming treatment to the below mentioned pet {getPatientDisplayName()} and documented all parasite control measures. The pet has received appropriate deworming medication as per veterinary standards and protocols.
            </Text>
      ...
      await downloadPDF(
        <DewormingPDF />,
        `deworming-certificate-${getPatientDisplayName()}-${new Date().toISOString().split('T')[0]}.pdf`,
        () => {
          toast({
            title: "Success",
            description: "Deworming Certificate downloaded successfully",
          });
        },
        (error) => {
          toast({
  ...
  const renderCertificatePreview = () => {
    ...
    const certificationText = "This is to certify that I have personally examined the below mentioned pet and administered appropriate deworming treatment. The pet has been treated for internal parasites and is free from worm infestation. Regular deworming schedule has been established for continued parasite control."
