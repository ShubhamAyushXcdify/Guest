...
export default function VaccinationCertificate({ appointmentId, patientId, onClose, readOnly = false }: VaccinationCertificateProps) {
    ...
    try {
      ...
      if (certificateData) {
        ...
        toast({
          ...
          description: "Vaccination Certificate updated successfully.",
        });
      } else {
        ...
        toast({
          ...
          description: "Vaccination Certificate saved successfully.",
        });
      }
    } catch (error: any) {
      toast({
  ...
  const handleDownloadCertificate = async () => {
    try {
      ...
      const VaccinationPDF = () => (
        <Document>
          <Page size="A4" style={certificateStyles.page}>
            ...
            {/* Title */}
            <Text style={certificateStyles.title}>VACCINATION CERTIFICATE</Text>

            {/* Main Content */}
            <Text style={certificateStyles.content}>
              This is to certify that I have personally administered vaccinations to the below mentioned pet {getPatientDisplayName()} and documented all immunization records. The pet has received all necessary vaccinations as per veterinary standards and protocols.
            </Text>
      ...
      await downloadPDF(
        <VaccinationPDF />,
        `vaccination-certificate-${getPatientDisplayName()}-${new Date().toISOString().split('T')[0]}.pdf`,
        () => {
          toast({
            title: "Success",
            description: "Vaccination Certificate downloaded successfully",
          });
        },
        (error) => {
          toast({
