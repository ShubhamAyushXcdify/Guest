...
useEffect(() => {
  ...
  const handleDownloadCertificate = async () => {
    try {
      ...
      await downloadPDF(
        <ConsentBondPDF />,
        `consent-bond-certificate-${getPatientDisplayName()}-${new Date().toISOString().split('T')[0]}.pdf`,
        () => {
          toast({
            title: "Success",
            description: "Consent Bond Certificate downloaded successfully",
          });
        },
        (error) => {
          toast({
