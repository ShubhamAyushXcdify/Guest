...
interface DncLeptoDocumentationModalProps {
  open: boolean;
  onClose: () => void;
  vaccine: any;
  patientId: string;
...
export default function DncLeptoDocumentationModal({ open, onClose, vaccine, appointmentId, clinicId }: DncLeptoDocumentationModalProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent side="right" className="w-full sm:!max-w-full md:!max-w-[50%] lg:!max-w-[50%] overflow-x-hidden overflow-y-auto">
        <SheetTitle asChild>
          <h2 className="text-2xl font-bold">Leptospira Vaccination Record</h2>
        </SheetTitle>
        <form className="p-6 space-y-6" onSubmit={handleSubmit}>
          <p className="text-gray-600">Custom documentation for Leptospira spp. (dog, non-core)</p>
