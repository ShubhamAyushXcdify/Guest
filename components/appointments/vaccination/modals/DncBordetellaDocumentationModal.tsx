...
interface DncBordetellaDocumentationModalProps {
  open: boolean;
  onClose: () => void;
  vaccine: any;
  patientId: string;
...
export default function DncBordetellaDocumentationModal({ open, onClose, vaccine, appointmentId, clinicId }: DncBordetellaDocumentationModalProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent side="right" className="w-full sm:!max-w-full md:!max-w-[50%] lg:!max-w-[50%] overflow-x-hidden overflow-y-auto">
        <SheetTitle asChild>
          <h2 className="text-2xl font-bold">Bordetella Vaccination Record</h2>
        </SheetTitle>
        <form className="p-6 space-y-6" onSubmit={handleSubmit}>
          <p className="text-gray-600">Custom documentation for Bordetella bronchiseptica (dog, non-core)</p>
