...
interface CanineCoronavirusDocumentationModalProps {
  open: boolean;
  onClose: () => void;
  vaccine: any;
  patientId: string;
...
export default function CanineCoronavirusDocumentationModal({
  open,
  onClose,
  vaccine,
  appointmentId,
  clinicId,
}: CanineCoronavirusDocumentationModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formattedDateGiven = dateGiven ? dateGiven.toISOString() : null;
    const formattedNextDueDate = nextDueDate ? nextDueDate.toISOString() : null;

    const payload = {
      visitId,
      vaccinationMasterId,
      vaccinationJson: JSON.stringify({
        dateGiven: formattedDateGiven,
    ...
    try {
      ...
      JSON.parse(payload.vaccinationJson);
      ...
      alert("Error creating vaccination record. Please check the data and try again.");
    }
  };
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      ...
      >
        <SheetTitle asChild>
          <h2 className="text-2xl font-bold">Canine Coronavirus Vaccination Record</h2>
        </SheetTitle>
        <form className="p-6 space-y-6" onSubmit={handleSubmit}>
          <p className="text-gray-600">Custom documentation for Canine Coronavirus vaccination</p>
