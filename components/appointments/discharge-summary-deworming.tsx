...
export default function DischargeSummarySheet({ 
  ...
  const { data: dischargeData, isLoading: isLoadingDischarge, error: dischargeError } = useGetDischargeSummaryDeworming(
    visitData?.id || "",
    isOpen && !!visitData?.id
  )
      ...
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.header}>
            <Text style={styles.title}>DEWORMING DISCHARGE SUMMARY</Text>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.grid}>
              ...
              <View style={styles.gridItem}>
                ...
                <Text style={styles.value}>{data.appointment?.appointmentType?.name || 'Deworming'}</Text>
              </View>
            </View>
          </View>

          {/* Deworming Information */}
          {data.dewormingDetail && (
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>DEWORMING INFORMATION</Text>
              <View style={styles.dewormingDetails}>
                <View style={styles.row}>
                  <Text style={styles.label}>Deworming Product:</Text>
                  <Text style={styles.value}>{data.dewormingDetail.productName || 'N/A'}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}>Dosage:</Text>
                  <Text style={styles.value}>{data.dewormingDetail.dosage || 'N/A'}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}>Administration Route:</Text>
                  <Text style={styles.value}>{data.dewormingDetail.route || 'N/A'}</Text>
                </View>
                <View style={styles.row}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.notes}>
              <Text>Please observe your pet for any adverse reactions over the next 24-48 hours.</Text>
              <Text>Follow the recommended deworming schedule for optimal parasite control.</Text>
              <Text>Contact the clinic immediately if your pet shows signs of vomiting, diarrhea, or lethargy.</Text>
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = `deworming-summary-${dischargeData?.patient?.name || 'patient'}-${new Date().toISOString().split('T')[0]}.pdf`
    ...
    toast({
      title: "Success",
      description: "Deworming discharge summary downloaded successfully",
    })
  }
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-gray-500">Loading deworming discharge summary...</p>
              </div>
            </div>
          ) : !visitData ? (
            <div className="flex items-center justify-center py-12">
              ...
              {pdfUrl && (
                <div className="border rounded-lg overflow-hidden">
                  <iframe
                    ...
                    title="Deworming Discharge Summary Preview"
                  />
                </div>
              )}
