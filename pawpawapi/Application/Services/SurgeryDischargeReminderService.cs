...
{
    public class SurgeryDischargeReminderService : ISurgeryDischargeReminderService
    {
        private readonly ISurgeryDischargeReminderRepository _reminderRepository;
        private readonly IEmailService _emailService;
        private readonly ILogger<SurgeryDischargeReminderService> _logger;
        private readonly int _reminderDaysBefore;
        private readonly string _bookingUrl;

        public SurgeryDischargeReminderService(
            ISurgeryDischargeReminderRepository reminderRepository,
            IEmailService emailService,
            ILogger<SurgeryDischargeReminderService> logger,
            IConfiguration configuration)
        {
            ...
            var reminderDaysStr = configuration["SurgeryDischargeReminder:ReminderDaysBefore"];
            ...
            _bookingUrl = configuration["SurgeryDischargeReminder:BookingUrl"] ?? "https://petpalace.xcdify.com/appointments";
        }

        public async Task<ReminderResponseDto> ProcessSurgeryDischargeRemindersAsync()
        {
            ...
            try
            {
                _logger.LogInformation("Starting surgery discharge reminder processing at {Time}", DateTimeOffset.UtcNow);
                ...
                List<SurgeryDischargeReminder> reminders;

                try
                {
                    reminders = await _reminderRepository.GetSurgeryDischargeRemindersDueAsync(today, futureDate);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to fetch surgery discharge reminders from database");
                    response.Message = "Failed to fetch surgery discharge reminders from database";
                    response.Errors.Add($"Database Error: {ex.Message}");
                    return response;
                }

                _logger.LogInformation("Found {Count} surgery discharge reminders to process (due within {Days} days)", reminders.Count, _reminderDaysBefore);
                response.TotalRemindersProcessed = reminders.Count;

                // Group reminders by days until due to send different emails
                var remindersByDays = new Dictionary<int, List<SurgeryDischargeReminder>>();
                ...
                foreach (var reminder in reminders)
                {
                    try
                    {
                        ...
                        {
                            remindersByDays[daysUntilFollowup] = new List<SurgeryDischargeReminder>();
                        }
                ...
                foreach (var group in remindersByDays)
                {
                    ...
                    foreach (var reminder in reminderList)
                    {
                        try
                        {
                            await SendSurgeryDischargeReminderEmailAsync(reminder, daysUntil);
                            response.EmailsSent++;
                            _logger.LogInformation(
                                "Successfully sent {Type} surgery discharge reminder to {Email} for patient {PatientName}",
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex,
                                "Failed to send surgery discharge reminder to {Email} for patient {PatientName}",
                                reminder.ClientEmail, reminder.PatientName);
                        }
                    }
                }
                ...
                if (response.Success)
                {
                    response.Message = $"Successfully processed {response.TotalRemindersProcessed} surgery discharge reminders and sent {response.EmailsSent} emails";
                }
                ...
                _logger.LogInformation("Completed surgery discharge reminder processing at {Time}. Success: {Success}, Sent: {Sent}, Failed: {Failed}",
            ...
            {
                _logger.LogError(ex, "Critical error processing surgery discharge reminders");
                response.Success = false;
                response.Message = "Critical error occurred during surgery discharge reminder processing";
        ...
        private async Task SendSurgeryDischargeReminderEmailAsync(SurgeryDischargeReminder reminder, int daysUntilFollowup)
        {
            ...
            if (daysUntilFollowup == 0)
            {
                ...
                emailSubject = $"🚨 URGENT: Surgery Follow-up Appointment Due TODAY for {reminder.PatientName}";
                headerIcon = "🚨";
                mainMessage = $"<strong>{reminder.PatientName}</strong>'s surgery follow-up appointment is <strong style='color: #d32f2f; font-size: 18px;'>DUE TODAY</strong>! Please bring your pet in for their follow-up appointment as soon as possible.";
            }
            else if (daysUntilFollowup == 1)
            {
                // Follow-up is TOMORROW
                urgencyMessage = "<strong style='color: #225F69;'>TOMORROW</strong>";
                emailSubject = $"⚠️ Reminder: Surgery Follow-up Appointment Due Tomorrow for {reminder.PatientName}";
                headerIcon = "⚠️";
                mainMessage = $"<strong>{reminder.PatientName}</strong>'s surgery follow-up appointment is <strong style='color: #225F69; font-size: 18px;'>DUE TOMORROW</strong>! Please ensure your pet is ready for their follow-up appointment.";
            }
            else
            {
                // Follow-up is in X days
                urgencyMessage = $"<strong style='color: #225F69;'>{daysUntilFollowup} days</strong>";
                emailSubject = $"📅 Reminder: Surgery Follow-up Appointment Due in {daysUntilFollowup} Days for {reminder.PatientName}";
                headerIcon = "📅";
                mainMessage = $"<strong>{reminder.PatientName}</strong>'s surgery follow-up appointment is due in <strong style='color: #225F69; font-size: 18px;'>{daysUntilFollowup} days</strong>. Please prepare for your pet's follow-up appointment.";
            }
...
<head>
    ...
    <title>Surgery Follow-up Reminder</title>
</head>
<body style='margin: 0; padding: 0; background-color: #f5f5f5; font-family: Arial, sans-serif;'>
    <table role='presentation' cellspacing='0' cellpadding='0' border='0' width='100%' style='background-color: #f5f5f5;'>
        <tr>
                            ...
                            <table role='presentation' cellspacing='0' cellpadding='0' border='0' width='100%'>
                                <tr>
                                    <td align='center' style='font-size: 32px; font-weight: bold; color: white;'>
                                        {headerIcon} Surgery Follow-up Reminder
                                    </td>
                                </tr>
                                <tr>
                                            ...
                                            <tr>
                                                <td style='padding: 20px;'>
                                                    <div style='font-size: 16px; color: #333; line-height: 1.5;'>
                                                        <strong>{reminder.PatientName}</strong>'s surgery follow-up is due <strong style='color: #225F69;'>{urgencyMessage}</strong>. Please schedule an appointment if you haven't already.
                                                    </div>
                                            ...
                                            <tr>
                                                <td style='padding: 15px;'>
                                                    <div style='font-size: 14px; color: #333; line-height: 1.5;'>
                                                        <strong style='color: #856404;'>⚠️ Important:</strong> Click the ""Book Appointment Now"" button above to schedule {reminder.PatientName}'s surgery follow-up appointment. Regular follow-up care is crucial for your pet's recovery and helps ensure the best possible outcome.
                                                    </div>
                                                </td>
                                            </tr>
                                        </table>
            ...
            if (!string.IsNullOrEmpty(reminder.MedicationsToGoHome))
            {
                emailContent += $@"
                                <!-- Medications -->
                                <tr>
                ...
                <table role='presentation' cellspacing='0' cellpadding='0' border='0' width='600' style='max-width: 600px;'>
                    <tr>
                        <td style='text-align: center; font-size: 12px; color: #666; line-height: 1.5;'>
                            <div>This is an automated surgery follow-up reminder. Please do not reply to this email.</div>
